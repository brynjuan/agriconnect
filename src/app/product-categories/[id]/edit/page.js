...
export default function EditCategory({ params }) {
  ...
  const fetchCategory = async () => {
    try {
      ...
      setApiError('Failed to fetch category details');
    }
  };
      ...
      <div className="w-full">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-semibold text-stone-800">Edit Kategori</h1>
          </div>

          <Card>
            <CardHeader title="Informasi Kategori" subtitle="Perbarui detail kategori" className="border-b border-stone-200" />
            
            <CardContent>
              {successMessage && (
                <div className="mb-6 p-4 border-l-4 border-stone-600 bg-stone-50 text-stone-600">
                  <p className="text-sm">{successMessage || "Kategori berhasil diperbarui!"}</p>
                </div>
              )}

              {apiError && (
                <div className="mb-6 p-4 border-l-4 border-red-500 bg-red-50 text-red-700">
                  <p className="text-sm">{apiError || "Gagal memproses permintaan"}</p>
                </div>
              )}
