...
export default function EditProductStockLog({ params }) {
  ...
  const fetchStockLog = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/product-stock-logs/${params.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch stock log: ${response.status}`);
      }
      
      const data = await response.json();
      
      if (data.status) {
        const stockLog = data.data.stockLog;
        setFormData({
        ...
        throw new Error(data.message || 'Failed to fetch stock log');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status) {
        ...
        throw new Error(data.message || 'Failed to update stock log');
      }
    } catch (error) {
        ...
        <div className="w-full">
          <div className="flex flex-col gap-6">
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <div className="text-center text-stone-500">Memuat...</div>
            </div>
          </div>
        </div>
      </AppLayout>
      ...
      <div className="w-full">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-semibold text-stone-800">Ubah Log Stok</h1>
          </div>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              {success && (
                <div className="mb-4 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
                  <p className="text-sm text-emerald-700">{success || "Log stok berhasil diperbarui"}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-10 pl-3 pr-10 border border-stone-200 rounded-lg text-sm bg-stone-50 cursor-not-allowed"
                    disabled={true}
                    required
                  >
                  ...
                  <p className="mt-1 text-xs text-stone-500">Produk tidak dapat diubah</p>
                </div>
